...
(define vec (make-bitvector (+ vec-limit 1) #f))
...
;; rather than setting nth step when hit on empty slot in array 
;; just set its bit to #t 
;;
;; e sg sm pg pm tg tm rg rm cg cm eg em dg dm)
(define (forever e sg sm pg pm tg tm rg rm cg cm eg em dg dm)
  (let ((code (hax (list e sg sm pg pm tg tm rg rm cg cm eg em dg dm)))
    ...
    (bitvector-set! vec code #t)
    (let ((n -1))
      
    (while (not (bitvector-ref/bool vec vec-limit))
      ;; next iteration
      (set! n (+ n 1))
      (set! processed 0)
      ;; swap 
      ...
      (for i 0 vec-limit ;; each entry
	   (let ((k (bitvector-ref/bool vec i)))
	     (when k  ;;(and (integer? k) (= k n)) ;;; hmmm- end up processing everything over nover
	       (incf processed)
	       (let ((c (hax i)))
		 (let ((e  (vector-ref c 0))
		       ...
		       (fn (lambda (e sg sm pg pm tg tm rg rm cg cm eg em dg dm n)
				 ...
				 (let ((val (bitvector-ref/bool vec i)))
				   (cond
				    ((not val)
				     (bitvector-set! vec i #t))))))))))
		   (cond
		    ((invalid e sg sm pg pm tg tm rg rm cg cm eg em dg dm) #f)
		    (#t 
    ...
    (format #t "~%found solution [ ~a : ~a ] ~%~%" (bitvector-ref vec vec-limit) n))))
