...
;; (define vec (make-vector vec-limit #f))
...
(define (forever e sg sm pg pm tg tm rg rm cg cm eg em dg dm)
  (let ((code (hax (list e sg sm pg pm tg tm rg rm cg cm eg em dg dm))))
    ...
    (let ((n -1)
      ...
      ;; add code to hash
      (hash-table-set! hash code 0)
      
    (while (not (hash-table-ref/default hash vec-limit #f))
      ;; next iteration
      (set! n (+ n 1))
      (set! processed 0)
      ;; swap 
      ...
      (hash-table-for-each hash
			   (lambda (key val)
			     ;;(format #t "hash has key = ~a : val = ~a ~%" key val)
			     (when (and (integer? key) (= val n))
			       (incf processed)
	       (let ((c (hax key)))
		 (let ((e  (vector-ref c 0))
		       ...
		       (fn (lambda (e sg sm pg pm tg tm rg rm cg cm eg em dg dm n)
				 ...
				 (let ((val (hash-table-ref/default hash i #f)))
				   (cond
				    ((not val)
				     (hash-table-set! hash i n))))))))))
		   (cond
		    ((invalid e sg sm pg pm tg tm rg rm cg cm eg em dg dm) #f)
		    (#t 
    ...
    (format #t "found solution ~a ~%" (hash-table-ref/default hash vec-limit #f)))))
